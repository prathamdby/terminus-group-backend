...
const uploadSingleFile = async (file?: Express.Multer.File): Promise<string | null> => {
...
const uploadMultipleFiles = async (files?: Express.Multer.File[]): Promise<string[]> => {
    if (!files || files.length === 0) return [];

    try {
        const uploadFiles = files.map(convertToUploadThingFile).filter((file): file is File => file !== null);
        const uploadResponses = await utapi.uploadFiles(uploadFiles);
        return uploadResponses.map((response) => response.data?.url).filter((url): url is string => url !== undefined);
    } catch (error) {
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            if (req.body.tags) {
                updateData.tags = req.body.tags.split(",").map((tag: string) => tag.trim());
            }

            if (req.body.consultants) {
                updateData.consultants = req.body.consultants.split(",").map((consultant: string) => consultant.trim());
            }
